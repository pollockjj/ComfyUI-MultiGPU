...
# Enable multi-GPU pipelining by default
USE_MULTIGPU_PIPELINE = True

# Setup GPU pipeline if multiple GPUs are available
if USE_MULTIGPU_PIPELINE and torch.cuda.device_count() > 1:
    # Main compute device (cuda:0)
    main_device = torch.device("cuda:0")
    # Prep device (cuda:1)
    prep_device = torch.device("cuda:1")
    
    # Create dedicated streams for each device
    main_stream = torch.cuda.Stream(device=main_device)
    prep_stream = torch.cuda.Stream(device=prep_device)
    
    # Create synchronization events
    main_event = torch.cuda.Event(enable_timing=False)
    prep_event = torch.cuda.Event(enable_timing=False)
    
    # Initialize pipeline state
    pipeline_initialized = False
    
    print("\n" + "="*80)
    print("ComfyUI-MultiGPU: GPU Pipeline ENABLED")
    print("Using cuda:0 for final compute with cuda:1 for preparation")
    print("="*80 + "\n")
else:
    print("\n" + "="*80)
    print("ComfyUI-MultiGPU: Single GPU mode (Pipeline disabled)")
    print("="*80 + "\n")

# Basic TorchScript optimized function for single GPU operation
@torch.jit.script
def apply_lora_patch_optimized(
    v0: torch.Tensor,             # v[0]
    v1: torch.Tensor,             # v[1]
    ...
    intermediate_dtype: torch.dtype,  # dtype for intermediate calculation
    output_dtype: torch.dtype     # final output dtype
) -> torch.Tensor:
    # Cast tensors to device 
    mat1 = v0.to(device=device, dtype=intermediate_dtype)
    mat2 = v1.to(device=device, dtype=intermediate_dtype)
    ...
    # Scale and convert to the right dtype
    result = weight + ((strength * alpha) * lora_diff).to(dtype=output_dtype)
    
    return result

# Pure pipeline implementation for LoRA application
# This processes the current tensor on cuda:0 while preparing the next on cuda:1
def apply_lora_patch_pipeline(
    v0, v1, weight, alpha, strength, 
    device, intermediate_dtype, output_dtype
):
    # Fall back to standard function if multi-GPU not available
    if not USE_MULTIGPU_PIPELINE or torch.cuda.device_count() <= 1:
        return apply_lora_patch_optimized(
            v0, v1, weight, alpha, strength, 
            device, intermediate_dtype, output_dtype
        )
    
    # Direct implementation - no caching, just pipelining
    
    # Record event on main stream to know when to start work on prep stream
    main_event.record(torch.cuda.current_stream())
    
    # Start work on prep stream (cuda:1)
    with torch.cuda.stream(prep_stream):
        # Wait for main stream event
        prep_stream.wait_event(main_event)
        
        # Move tensors to cuda:1 and perform computation
        mat1 = v0.to(device=prep_device, dtype=intermediate_dtype, non_blocking=True)
        mat2 = v1.to(device=prep_device, dtype=intermediate_dtype, non_blocking=True)
        
        # Do the expensive matrix multiplication on cuda:1
        lora_diff = torch.mm(
            mat1.flatten(start_dim=1), 
            mat2.flatten(start_dim=1)
        ).reshape(weight.shape)
        ...
        # Move result back to cuda:0 for final addition
        result_tensor = lora_diff.to(device=device, non_blocking=True)
        
        # Record completion event on prep stream
        prep_event.record(prep_stream)
    
    # Wait for prep stream to complete
    torch.cuda.current_stream().wait_event(prep_event)
    
    # Final addition on cuda:0
    result = weight + result_tensor
...
def calculate_weight(patches, weight, key, intermediate_dtype=torch.float32, original_weights=None):
    for p in patches:
        ...
        if patch_type == "diff":
            ...
            # Skip optimized path if we have locon or other complex features
            dora_scale = v[4]
            reshape = v[5]
            
            if reshape is not None or v[3] is not None or dora_scale is not None:
                ...
                # Use optimized path for standard LoRA
                if v[2] is not None:
                    alpha = v[2] / v[1].shape[0]
                else:
                    alpha = 1.0
                    
                try:
                    # Use the GPU pipeline implementation
                    weight = apply_lora_patch_pipeline(
                        v[0], v[1], weight, alpha, strength, 
                        weight.device, intermediate_dtype, weight.dtype
                    )
                except Exception as e:
                    logging.error("ERROR optimized lora {} {}".format(key, e))
                    ...
                    try:
                        ...
                        logging.error("ERROR fallback lora {} {}".format(key, e2))
